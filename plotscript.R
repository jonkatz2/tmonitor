...
ymin <- min(unlist(lapply(dat[5], min, na.rm=TRUE)))
ymax <- max(unlist(lapply(dat[5], max, na.rm=TRUE)))
plot(1:nrow(dat), dat[,5], type='l', xaxt='n', xlab=NA, ylab=expression(paste('Temp (', degree, 'F)')), main='Ambient', ylim=c(ymin, ymax))
